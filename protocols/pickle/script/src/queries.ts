...
import { PICKLE_VESTING_SUBGRAPH, DELAY } from './constants';
import { MasterchefPool, PicklePool, PickleUser } from "../types/subgraphs/pickle_vesting";
...
// Fetches Pickle's pool data
async function picklePoolsQuery(blocks: number[]) {
    const queries = [];
    ...
    const picklePools: PicklePool[][] = await Promise.all(queries);

    return picklePools.map(block => 
        block
            .map(pool => ({
                picklePoolAddress: String(pool.id),
...
async function usersQuery(blocks: number[]) {
    ...
    const users: PickleUser[][] = await Promise.all(queries);

    return users.map(block => 
        block
            .map(user => ({
                user: user.address!,
                picklePoolAddress: user.picklePool?.id!,
...
export default async function query({startBlock, endBlock, step}: {startBlock: number, endBlock: number, step: number}) {  
    ...
    const picklePools = await picklePoolsQuery(blocks);
    const users = await usersQuery(blocks);

    return {
        poolsData: picklePools.map((block, i) => {
            ...
            return (
                block.map(picklePool => {
                    const masterchefPool = masterchefBlock.find(masterchefPool => masterchefPool.id === picklePool.masterchefPoolId)!;
                
                    // Calculates the weight of the pickle pool compared to the total weight of Masterchef
                    const weight = picklePool.balance.times(picklePool.pricePerShare).dividedBy(masterchefPool.balance).times(masterchefPool.weight);

                    return ({
                        picklePoolAddress: picklePool.picklePoolAddress,
                        pricePerShare: picklePool.pricePerShare,
