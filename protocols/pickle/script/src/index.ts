...
const JAR_POOL_MAP = require('./constants').JAR_POOL_MAP;



export default async function getPickleDistribution(options: Options) {
    ...
    poolsData.forEach((block, i) => {
        ...
        block.forEach(pool => {
            const users = usersBlock.filter(user => user.picklePoolAddress === pool.picklePoolAddress);

            // Looks dumb, but required, otherwise the forEach under it would fail
            balances[pool.picklePoolAddress] = balances[pool.picklePoolAddress] ?? {"0": new BigNumber(0)};
            ...
            users.forEach(user => {
                ...
                balances[pool.picklePoolAddress][user.user] = balances[pool.picklePoolAddress][user.user] ? balances[pool.picklePoolAddress][user.user].plus(points) : points;
            });
        });
    });
    ...
    const usersWithDuplicates = Object.keys(balances).map(poolKey => {
        ...
        // Has to be translated through a map because of Pickle's contract design
        const fraction = new BigNumber(options.blacklistDistribution[JAR_POOL_MAP[poolKey]]).div(totalPoints);
